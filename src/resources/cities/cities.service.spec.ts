...
describe('city resource lifecycle', () => {
  it('creates cities and returns only enabled options', () =>
    withResourceScope(
      ...
      async () => {
        ...
        expect(repository.find).toHaveBeenCalledWith(
          expect.objectContaining({
            where: { status: ResourceStatus.Enabled, library: 'shengxu' },
          }),
        );
      },
    ));
  it('updates status under a version lock and rejects renaming a city', () =>
    withResourceScope(
      { resourceLibrary: 'shengxu' } as AuthenticatedUser,
      null,
      async () => {
        ...
        const service = new CitiesService(
          repository as unknown as Repository<CityEntity>,
          {
            transaction: (fn: (manager: EntityManager) => unknown) =>
              fn(manager),
          } as DataSource,
        );
        await expect(
