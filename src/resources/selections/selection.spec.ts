...
describe('resource selections', () => {
  it('defaults to 10 and rejects invalid or unbounded pages', () =>
    withResourceScope(
  ...
  it('pages a joined price query instead of fetching each resource', () =>
    withResourceScope(
      { resourceLibrary: 'shengxu' } as AuthenticatedUser,
      null,
      async () => {
        ...
        await expect(
          service.detail('attraction', 'missing'),
        ).rejects.toMatchObject({ code: 'ATTRACTION_PRICE_NOT_FOUND' });
